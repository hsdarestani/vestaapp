(() => {
  const paths = {
    ...
    sparkles:'<path d="m12 3-1.6 3.8L7 8.4l3.4 1.6L12 14l1.6-4 3.4-1.6-3.4-1.6Z"/><path d="m19 15-.8 1.9-1.7.8 1.7.8.8 2 .8-2 1.7-.8-1.7-.8Z"/><path d="m5 14-.7 1.6-1.4.7 1.4.7.7 1.6.7-1.6 1.4-.7-1.4-.7Z"/>',
    ...
    droplets:'<path d="M7 16.3c2.2 0 4-1.8 4-4 0-2-2.7-5.4-4-6.8-1.3 1.4-4 4.8-4 6.8 0 2.2 1.8 4 4 4Z"/><path d="M17 21c2.2 0 4-1.8 4-4 0-2-2.7-5.4-4-6.8-1.3 1.4-4 4.8-4 6.8 0 2.2 1.8 4 4 4Z"/>',
  ...
  function svg(name){
    ...
    return `<svg class="v-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${body}</svg>`;
  }
  ...
  function upgradeProducts(){
    document.querySelectorAll('.product-card').forEach(card => {
      ...
      box.innerHTML = svg(productIcons[title] || 'shopping-bag');
    });
  }
  ...
  function upgradeMedia(){
    document.querySelectorAll('.photo-placeholder').forEach(box => {
      ...
      box.innerHTML = `<div class="media-placeholder-inner">${svg('image')}<span>${label}</span></div>`;
    });
  }
  ...
  async function confirmPayment(){
    ...
    const token = localStorage.getItem('vestaland:token') || '';
    if(!token || !receipt || !intent){ history.replaceState({},'',location.pathname); return; }
    try{
  ...
  document.addEventListener('DOMContentLoaded', () => {
    renderIcons(); upgradeProducts(); upgradeMedia(); confirmPayment(); paymentDoneToast();

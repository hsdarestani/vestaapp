(() => {
  ...
  const svg=n=>`<svg class="v-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths[n]||paths.sparkles}</svg>`;
  ...
  async function startPayment(plan,button){const token=localStorage.getItem('vestaland:token')||'';if(!token){window.showAuthModal?.('login');return}const old=button.innerHTML;button.disabled=true;button.innerHTML='<span><b>در حال انتقال به درگاه…</b></span>';try{const r=await fetch('/api/payments/start',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`},body:JSON.stringify({plan})}),d=await r.json();if(!r.ok)throw new Error(d.error||'شروع پرداخت انجام نشد.');location.href=d.url}catch(err){button.disabled=false;button.innerHTML=old;window.toast?.(err.message)}}

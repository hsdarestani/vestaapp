(() => {
  if (window.__vestalandMarketPayment) return;
  window.__vestalandMarketPayment = true;
  ...
  function showForm(store){
    ...
    openModal(`<p class="kicker">پرداخت ${storeName(store)}</p><h2>آدرس تحویل</h2>
      ...
      <form id="marketCheckoutForm" class="market-checkout-form" data-store="${esc(store)}">
        ...
        <p id="marketCheckoutErr" class="form-error hidden"></p>
  ...
  async function startPayment(form){
    ...
    const button=form.querySelector('button[type=submit]'),err=$('#marketCheckoutErr');
  ...
  document.addEventListener('submit',e=>{
    const f=e.target.closest?.('#marketCheckoutForm');if(!f)return;

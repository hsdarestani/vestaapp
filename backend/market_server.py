...
MAX_BODY = 128 * 1024
UA = 'Mozilla/5.0 (Linux; Android 14) AppleWebKit/537.36 Chrome/131 Safari/537.36 VestalandMarket/1.0'
SSL = ssl.create_default_context()


def request_json(url, method='GET', body=None, headers=None, timeout=18):
...
def product_payload(store, p):
    ...
    return {
        ...
        'images': [{'src': x.get('src') or '', 'alt': x.get('alt') or x.get('name') or ''} for x in images[:8]],
...
def fetch_categories(store):
    ...
    return [{
        'store': store, 'id': int(x.get('id') or 0), 'name': x.get('name') or '',
        'slug': x.get('slug') or '', 'count': int(x.get('count') or 0),
        'image': ((x.get('image') or {}).get('src') if isinstance(x.get('image'), dict) else '') or '',
    } for x in data]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def read_json(self):
        ...
        if length <= 0 or length > MAX_BODY: raise ValueError('درخواست نامعتبر است.')
        ...
        try:
            ...
            if path == '/api/market/products':
                ...
                if store == 'all':
                    ...
                    items = []
                    for r in results: items.extend(r['items'])
                    items.sort(key=lambda x: x.get('id',0), reverse=True)
                    return self.send_json(200, {'items': items, 'stores': results, 'errors': errors})
                return self.send_json(200, fetch_products(store, qs))
                ...
                if store == 'all':
                    with ThreadPoolExecutor(max_workers=2) as ex:
                        ...
                        for s, rows in zip(STORES, ex.map(fetch_categories, STORES)):
                            out.extend(rows)
                    return self.send_json(200, {'categories': out})
                return self.send_json(200, {'categories': fetch_categories(store)})
            if path == '/api/market/product':
                store = qs.get('store','')
                pid = int(qs.get('id') or 0)
                return self.send_json(200, {'product': fetch_product(store,pid)})
...
def main():
    ...
    print(f'Vestaland market API listening on {args.host}:{args.port}', flush=True)
